...
func _ready() -> void:
	...
	$StaticBody2D/topchecker/CollisionShape2D.position = Vector2(-0.5,-2.5)
	sprite.play("up")
...
func _activate_button() -> void:
	...
	$StaticBody2D/topchecker/CollisionShape2D.position = Vector2(0,3)
	$buttonclick.play()
	$"../main".get_node("canon").shoot()
